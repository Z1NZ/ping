...
static void ft_out(int i)
{
	...
	if (g_env.seq) {
		if (g_env.recvpack > g_env.seq)
			...
			printf("%d%% packet loss, time=%lums",
				   (int)(((g_env.seq - g_env.recvpack) * 100) / g_env.seq),
				   (g_env.timer / 10000));
	}
	printf("\n");
	if (g_env.recvpack > 0) {
		printf("rtt min/avg/max = %ld.%ld/%lu.%ld/%ld.%ld/", g_env.tmin / 1000,
			   g_env.tmin % 1000, (g_env.tsum / (g_env.recvpack)) / 1000,
			   (g_env.tsum / (g_env.recvpack)) % 1000, g_env.tmax / 1000,
			   g_env.tmax % 1000);
...
static void ft_ping_old(int i)
{
	...
	g_env.pack->hdr.checksum =
		ft_in_cksum((unsigned short *)g_env.pack, sizeof(struct packet));
	...
	if (sendto(g_env.sd, g_env.pack, PACKETSIZE, 0, (struct sockaddr *)&server,
...
jump:
	...
	if (len) {
		...
		if (p->hdr.icmp_hun.ih_idseq.icd_id != getpid() &&
			(p->hdr.icmp_code != ICMP_ECHOREPLY &&
			 p->hdr.icmp_code != ICMP_FILTER))
			goto jump;
		if (p->hdr.icmp_code == ICMP_FILTER) {
			alarm(0);
			++g_env.error;
			printf("From %d.%d.%d.%d icmp_seq=%ld %s\n",
				   (int)(p->ip.ip_src.s_addr & 0xFF),
				   (int)((p->ip.ip_src.s_addr & 0xFF00) >> 8),
				   (int)((p->ip.ip_src.s_addr & 0xFF0000) >> 16),
				   (int)((p->ip.ip_src.s_addr & 0xFF000000) >> 24), g_env.seq,
				   "Packet filtered");
		} else {
			...
			if ((CHECK_BIT(g_env.opt, OPT_T)) && p->ip.ip_ttl > g_env.ttl) {
				if (!CHECK_BIT(g_env.opt, OPT_Q))
					printf("%zd bytes from %s: icmp_seq=%u %s\n",
						   msg.msg_iov[0].iov_len, g_env.name,
						   p->hdr.icmp_hun.ih_idseq.icd_seq,
				...
				if (!CHECK_BIT(g_env.opt, OPT_Q))
					printf(
						"%zd bytes from %s: icmp_seq=%u ttl=%d time=%.2fms\n",
						msg.msg_iov[0].iov_len, g_env.name,
						p->hdr.icmp_hun.ih_idseq.icd_seq, p->ip.ip_ttl,
						tmp / 1000);
				g_env.tsum += tmp;
				g_env.tsum2 +=
